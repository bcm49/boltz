#!/usr/bin/env python3
"""
Find allosteric pockets using Fpocket on apo (ligand-free) protein structures
"""
...
def parse_fpocket_output(info_file_path):
    ...
    try:
        ...
        # Only match valid float format (digits, optional decimal, optional sign, scientific notation)
        pattern = r'Pocket\s+(\d+)\s+:.*?Score\s+:\s+([-+]?\d+\.?\d*(?:[eE][-+]?\d+)?)'
        ...
        if os.path.exists(pockets_pqr):
            with open(pockets_pqr, 'r') as f:
                for line in f:
                    if line.startswith('ATOM') and 'STP' in line:  # Alpha spheres
                        ...
                        if len(parts) >= 8:
                            try:
                                pocket_id = int(parts[4])  # Pocket ID is in residue number field
                                x, y, z = float(parts[5]), float(parts[6]), float(parts[7])
                                if pocket_id not in pocket_centers:
                                    pocket_centers[pocket_id] = []
                                pocket_centers[pocket_id].append([x, y, z])
                            except ValueError:
                                # Handle fpocket bug where negative numbers run together: "-42.078-104.952"
                                # Try to parse using fixed-width columns instead
                                try:
                                    pocket_id = int(line[22:26].strip())
            ...
            print(f"      ⚠️  PQR file not found: {pockets_pqr}")
        
        # Debug: Print what we found (only for first file)
        if 'DEBUG_FIRST' not in globals():
            globals()['DEBUG_FIRST'] = True
            print(f"      DEBUG: Found {len(pocket_centers)} pockets in PQR")
            if pocket_centers:
                sample_pocket = list(pocket_centers.keys())[0]
                print(f"      DEBUG: Pocket {sample_pocket} has {len(pocket_centers[sample_pocket])} alpha spheres")
        ...
        for match in matches:
            ...
            try:
                ...
                # Handle malformed scores (shouldn't happen but just in case)
                print(f"      WARNING: Skipping pocket {pocket_id} with malformed score: {match[1]}")
                continue
