...
def write_pdb(receptor_path, ligand_path, complex_path) -> None:
    subprocess.run(["conda", "run", "-n", "pymol", "pymol", "-cq", Path("hackathon/merge_complex.py"), "--", receptor_path, ligand_path, complex_path])
    

def prepare_and_dock(
    pdb_path, receptor_output_name, ligand_output_path, smiles, center, box_size, num_modes: int = 3
) -> list[Path]:
    ...
    write_pdb(receptor_path, ligand_output_path.with_name(f"{ligand_output_path.stem}_out.pdbqt"), complex_name)
    return [complex_name.with_name(f"{complex_name.name}_model_{i}.pdb") for i in range(1, num_modes+1)]
